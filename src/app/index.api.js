(function ()
{
    ...
    function apiService($resource)
    {
        ...
        api.execution = {
            list: $resource(api.baseUrl + 'exe-list.json'),
            detail: $resource(api.baseUrl + 'exe-detail.json')
        }

        api.company = {
            detail: $resource(api.baseUrl + 'company-info.json')
        }

        api.plan = {
            docs: $resource(api.baseUrl + 'plan-docs.json'),
            scripts: $resource(api.baseUrl + 'plan-script.json'),
            basic: $resource(api.baseUrl + 'plan-info.json'),
            docHistory: $resource(api.baseUrl + 'doc-history.json'),
            unfinishedPlan: $resource(api.baseUrl + 'unfinished-plan.json')
        }

        api.auth = {
        // mock server logic
        login: function(params){
            return $resource(api.baseUrl + 'user.json').get().$promise
               .then(function(result){
                  var users = result.data
                  for (var i = users.length - 1 ; i >= 0 ; i--){
                    if(users[i].username === params.username && users[i].password === params.password)
                        return Promise.resolve({data: users[i], status:'SUCCESS', timestamp: result.timestamp})
                    if(users[i].username === params.username && users[i].password !== params.password)
